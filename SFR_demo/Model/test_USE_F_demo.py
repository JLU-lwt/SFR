...
@hydra.main(version_base=None, config_path="config", config_name="USE_F")
def test(cfg):
    ...
    Fs_info=pd.read_csv(f"SFR/data/{cfg.testdata_path}.csv").values[:,:]
    ...
    for count,F_info in enumerate(Fs_info):
        ...
        if cfg.Draw:
            if dim==1:
                ...
                plt.savefig(f"SFR/results/Figure_USE_F_{count}.png",bbox_inches="tight")
